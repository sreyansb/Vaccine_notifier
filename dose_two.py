        ...
        try:
            if final["sessions"]:
                for i in final["sessions"]:
                        if i['available_capacity_dose2'] and i["vaccine"]=="COVAXIN" and i['min_age_limit']==18 :
                            print(i["name"],i["pincode"],i['available_capacity_dose2'])
                            engine.say((i["name"],i['available_capacity_dose2']))
                            engine.runAndWait()
                            winsound.Beep(750,800)    
                            di.append(({"D":f,"N":i['name'],"Cap":str(i['available_capacity_dose2']),"Add":i['address'],"F":i['fee'],"V":str(i["vaccine"]),"S":i["session_id"],"sl":i["slots"]}))
                            print(di[-1])
                    #print(di)
        except:
            engine.say("ERROR")
